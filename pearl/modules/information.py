...
@pearl.on(pearl_on_cmd("info ?(.*)"))
@pearl.on(sudo_cmd("info ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    ...
    replied_user_profile_photos_count = "K"
    try:
    ...
    try:
        ...
        str(e)
    caption = """<b>Extracted Userdata From Telegram DATABASE By Black Pearl<b>
