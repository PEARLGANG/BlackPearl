...
async def _(event):
    ...
    if event.pattern_match.group(1):
        ...
        await edit_or_reply(event, "`Am I Supposed To Find Any Kind Of Lyric? `")
        return
...
@pearl.on(pearl_on_cmd(outgoing=True, pattern="glyrics(?: |$)(.*)"))
async def lyrics(lyric):
    if r"-" in lyric.text:
        ...
        await lyric.edit(
            ...
            "eg: `.glyrics Rae Sremmurd - Powerglide`"
        )
        return

    if GENIUS is None:
        await lyric.edit(
            "`Provide genius access token to config.py or Heroku Var first K?`"
        )
    else:
