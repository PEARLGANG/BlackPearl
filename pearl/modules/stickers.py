...
async def _(event):
    ...
    # packname = f"BLACK PEARL"
    # packshortname = f"BLACKPEARL_{userid}_ns"  # format: Uni_Borg_userid
    if userid == 965670914:
        packname = f"@BlackPearl Packs 🎭"
        packshortname = "BlackPearlPack"
    else:
        packname = f"@{user.username} KangPack {pack}"
        packshortname = f"Pearl_{userid}_Pack"
    await moods.edit("`This Sticker is Gonna Get Stolen.....`")

    is_a_s = is_it_animated_sticker(reply_message)
    file_ext_ns_ion = "@Pearl.png"
    ...
    if is_a_s:
        ...
        if userid == 965670914:
            packname = f"Brought you by Black Pearl"
            packshortname = "k"
        else:
            packname = f"@{user.username} KangPack {pack}"
            packshortname = f"Pearl_{userid}"  # format: Uni_Borg_userid
...
@pearl.on(pearl_on_cmd(pattern="packinfo"))
@pearl.on(sudo_cmd(pattern="packinfo ?(.*)", allow_sudo=True))
async def _(event):
...
@pearl.on(pearl_on_cmd(pattern="getsticker ?(.*)"))
async def _(event):
    if event.fwd_from:
        return
