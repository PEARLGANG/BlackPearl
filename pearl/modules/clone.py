...
@pearl.on(pearl_on_cmd(pattern="clone ?(.*)"))
async def _(event):
    if event.fwd_from:
        return
    ...
    if first_name is not None:
        # some asshole's (like me) have more than 4096 characters in their names
    ...
    await event.delete()
    await borg.send_message(
        event.chat_id, "**THE WAR LEFT ITS SCARS ON ALL OF US**", reply_to=reply_message
    )
