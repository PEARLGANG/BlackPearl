...
from pearl import CMD_HELP, bot
...
async def fban(event):
    ...
    try:
        from pearl.modules.sql_helper.fban_sql import get_flist
    ...
    if fban_id == self_user.id or fban_id == "@" + self_user.username:
        return await event.edit(
            "**Error: This action has been prevented by BlackPearl self preservation protocols.**"
        )
...
async def addf(event):
    ...
    try:
        from pearl.modules.sql_helper.fban_sql import add_flist
    except IntegrityError:
        return await event.edit("**Running on Non-SQL mode!**")
...
async def delf(event):
    ...
    try:
        from pearl.modules.sql_helper.fban_sql import del_flist
    except IntegrityError:
        return await event.edit("**Running on Non-SQL mode!**")
...
async def delf(event):
    ...
    try:
        from pearl.modules.sql_helper.fban_sql import del_flist_all
    except IntegrityError:
        return await event.edit("**Running on Non-SQL mode!**")
